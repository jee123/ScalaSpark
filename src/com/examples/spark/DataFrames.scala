...
/**
 * algo:
 *  - convert fakefriends.csv to dataset using Person case class.
 *  - using this dataset infer schema and run queries.
 */
...
object DataFrames {
  ...
  def main(args: Array[String]) {
    ...
    println("Show people with age < 21:")
    people.filter(people("age") < 21).show()
   
    println("Group by age:")
    people.groupBy("age").count().show()
    
    println("Updating age by 10:")
